# OTP Login Authentication via Email (OTP) — OMP / OJS Plugin
...
- **Role-based configuration**: administrators can choose which roles must use OTP (site admin, manager, editor, reviewer, author, etc.) — or disable it entirely
...
After enabling the plugin, click **Settings** to choose which roles must complete the OTP step.

| Setting | Behaviour |
|---|---|
| One or more roles selected | Only users with those roles are required to verify via email |
| No roles selected | OTP is disabled for all users |
| Never configured (default) | OTP is required for all users |
